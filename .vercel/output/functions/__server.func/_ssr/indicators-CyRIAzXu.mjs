//#region node_modules/.nitro/vite/services/ssr/assets/indicators-CyRIAzXu.js
...
function structureBias(values) {
	if (values.length < 12) return "balanced";
	const recent = values.slice(-12);
	const first = recent[0];
	const last = recent.at(-1);
	const highs = recent.filter((_, i) => i > 0 && recent[i] > recent[i - 1]);
	if (last > first * 1.01 && highs.length >= 6) return "uptrend";
	if (last < first * .99) return "downtrend";
	return "range";
}
...
function betaVsMarket(stock, market) {
	...
	return Math.max(-.2, Math.min(3.5, beta));
}
//#endregion
export { sma as a, rsi as i, momentum as n, structureBias as o, realizedVol as r, betaVsMarket as t };
