...
import type { AgentType, Horizon, Reco } from "./types";
...
function weekPosition(s: AgentSnapshot) {
  const span = s.fiftyTwoWeekHigh - s.fiftyTwoWeekLow;
  if (span <= 0) return 0.5;
  return (s.last - s.fiftyTwoWeekLow) / span;
}

function localQuant(input: RunAgentInput): RunAgentResult {
  ...
  switch (agentType) {
    case "technical": {
      if (s.rsi < 32 && s.last > s.sma20) {
        ...
        thesis = `RSI ${s.rsi.toFixed(0)}, SMA10 ${s.sma10.toFixed(2)} vs last ${s.last.toFixed(2)}. No edge beyond noise — stand aside.`;
      }
      break;
    }
    case "fundamental": {
      const quality = ["Banks", "Energy", "Utilities", "Telecom", "Staples"].includes(s.sector);
      if (quality && pos52 < 0.35 && s.mom20 >= -6) {
        reco = "BUY";
        confidence = 63;
      } else if (pos52 > 0.92 && s.rsi > 70) {
        reco = "SELL";
        confidence = 58;
      } else {
        reco = "HOLD";
      ...
      thesis = quality
        ? `${s.nameEn} is a core TASI quality sleeve (${s.sector}). Last ${s.last.toFixed(2)} sits ${(pos52 * 100).toFixed(0)}% through the 52-week range (${s.fiftyTwoWeekLow.toFixed(2)}–${s.fiftyTwoWeekHigh.toFixed(2)}).`
        : `${s.sector} needs a tighter catalyst. 52-week range position ${(pos52 * 100).toFixed(0)}% does not justify adding size.`;
      horizon = "position";
      break;
    }
    case "news": {
      ...
      thesis = s.headlines[0]
        ? `Lead wire: “${s.headlines[0]}”. Desk maps this as ${reco.toLowerCase()} flow, not a thesis change.`
        : "No fresh name-specific wire. News agent stays flat.";
      horizon = "intraday";
      break;
    }
    case "sentiment": {
    ...
    case "macro": {
      const oilBid = s.brentChangePct > 0.6 && s.sector === "Energy";
    ...
    case "correlation": {
      const peerAvg =
        s.peers.reduce((a, p) => a + p.changePct, 0) / Math.max(1, s.peers.length);
      const gap = s.changePct - peerAvg;
      reco = gap < -0.8 && s.beta < 1.2 ? "BUY" : gap > 1.2 ? "SELL" : "HOLD";
      confidence = 55;
      thesis = `${s.symbol} vs TASI ${s.tasiChangePct.toFixed(2)}% / name ${s.changePct.toFixed(2)}% / sleeve ${peerAvg.toFixed(2)}% (β ${s.beta.toFixed(2)} vs TASI). ${
        reco === "BUY" ? "Lagging a firm book — relative-value bid." : reco === "SELL" ? "Rich to the sleeve." : "In line with the tape."
      }`;
      break;
    }
    ...
    case "regime": {
      ...
      confidence = 60;
      thesis = `Annualized realized vol ~${s.realizedVol.toFixed(1)}% on the Yahoo daily tape. ${
        s.realizedVol > 28 ? "Stress regime — cut gross." : s.realizedVol < 14 ? "Compression — trend-following longs work." : "Mid-vol, mixed."
      }`;
      horizon = "swing";
      break;
    }
    case "risk": {
      if (s.position && s.position.pnlPct < -8) {
        ...
        confidence = 50;
        thesis = s.position
          ? `Size ${s.position.qty.toLocaleString()} @ ${s.position.avg.toFixed(2)} is within desk limits (live mark ${s.last.toFixed(2)}).`
          : "No inventory. Risk is optional — only enter with a defined stop on the live last.";
      }
    ...
    case "self_healing": {
      ...
      thesis =
        open > 0
          ? `${open} circuit breaker(s) open, ${errs} unresolved. Reset, then wait for a clean tape poll before rerunning agents.`
          : `Breakers closed. ${errs} unresolved log lines. Live feed is healthy — no recovery action required.`;
...
function parseModel(text: string): Partial<RunAgentResult> | null {
  ...
  try {
    ...
    const horizon: Horizon =
      horizonRaw === "intraday" || horizonRaw === "position" ? horizonRaw : "swing";
    return {
      recommendation: reco,
...
export const runDeskAgent = createServerFn({ method: "POST" })
  .validator((data: RunAgentInput) => data)
